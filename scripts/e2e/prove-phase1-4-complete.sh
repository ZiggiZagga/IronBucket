...
wait_minio_ready() {
  ...
  for ((attempt=1; attempt<=max_attempts; attempt++)); do
    if curl -kfsS "${PHASE4_MINIO_SCHEME}://127.0.0.1:${PHASE4_MINIO_PORT}/minio/health/live" >/dev/null 2>&1; then
      return 0
    fi
    sleep 2
  done
...
if [[ "$PHASE4_MINIO_READY" == "true" ]]; then
  set +e
  (
    export IRONBUCKET_MINIO_ENDPOINT="${PHASE4_MINIO_SCHEME}://${PHASE4_MINIO_CONTAINER}:9000"
