...
GATEWAY_HEALTH=$(curl -s -o /dev/null -w "%{http_code}" https://localhost:8080/health)
...
UPLOAD_RESULT=$(curl -s -X PUT \
  ...
  https://localhost:8080/s3/ironbucket/test-document.txt \
  -w "%{http_code}")
...
curl -s https://localhost:8080/s3/ironbucket/test-document.txt 2>&1
...
VERIFY_DELETE=$(curl -s -w "HTTP %{http_code}" https://localhost:8080/s3/ironbucket/test-document.txt 2>&1)
echo "$VERIFY_DELETE"
